...
def match_parcels(gdf_1: gpd.GeoDataFrame, gdf_2: gpd.GeoDataFrame, index_1: rtree.Index, index_2: rtree.Index):
    ...
    for i, poly1 in enumerate(tqdm.tqdm(gdf_1.geometry, desc='Matching Parcels')):
...
def compare_polygons(poly_1: Polygon, poly_2: Polygon, fast_mode: bool=True):
    ...
    def list_points(polygon):
        norm_poly = normalize_polygon(polygon)
        x, y = norm_poly.exterior.coords.xy
        
        if len(x) > 1024:
            x, y = norm_poly.simplify(GRID_SIZE/2).exterior.coords.xy
            
        return [(x_, y_) for x_, y_ in zip(x, y)]
    
    if fast_mode: # Can cause bugs on certain datasets
        dist, _, _, _ = turning_function.distance(list_points(poly_1), list_points(poly_2))
    else:
        dist = run_turning_function_subprocess(list_points(poly_1), list_points(poly_2))
    
    return dist

def run_turning_function_subprocess(arg1, arg2, timeout=1):
    try:
        # Prepare the command to run the function in a separate Python process
        process = subprocess.Popen(
            [sys.executable, '-c', f'''
import json
import turning_function
dist, _, _, _ = turning_function.distance({arg1}, {arg2})
...
'''], 
        ...
        try:
            ...
            print("Function compare_polygons took too long, killing subprocess...")
            process.kill()
            return None
...
def create_image(polygons: Polygon, bounds: tuple, grid_size: int = 2, max_score: float = 1.):
    ...
    for x in tqdm.tqdm(range(width), desc='Drawing results'):
...
def compute_dynamics(rev_matches: dict, gdf_1: gpd.GeoDataFrame, gdf_2: gpd.GeoDataFrame):
    ...
    for i, key in tqdm.tqdm(enumerate(rev_matches.keys()), desc='Computing Fusion/Division Dynamics'):
            
        poly_2 = gdf_2['geometry'].iloc[int(key)]
...
def convert_multipolygon_to_polygon(multipolygon: MultiPolygon):
    ...
    # In case of failure (e.g. very thin geometry, simply take the largest part)
    if isinstance(polygon, MultiPolygon):
        areas = np.array([part.area for part in polygon.geoms])
...
df_pers_fusion = df_pers_fusion[df_pers_fusion['dist'] < 2.5]
df_pers_division = df_pers_division[df_pers_division['dist'] < 2.5]
...
img_fusion = create_image(fusion_dict, bounds, grid_size=GRID_SIZE)

for item in division_dict:
    item['polygon'] = gdf_1.iloc[item['2']]['geometry']
img_division = create_image(division_dict, bounds, grid_size=GRID_SIZE)
...
for i in range(light_img.shape[0]):
